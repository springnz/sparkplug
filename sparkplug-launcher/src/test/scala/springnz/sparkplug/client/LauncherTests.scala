...
class LauncherTests extends WordSpec with ShouldMatchers with Logging {
  ...
  "simple spark launch" should {
    "launch spark letter count process (without Akka)" in {
      val launcher = Launcher.launch("", root / System.getProperty("user.dir") / "target/pack/lib", "**/sparkplug-examples*", "springnz.sparkplug.examples.LetterCount", ClientExecutor.defaultSparkConfig)
